...
- **Produção / Easypanel**: `docker-compose.prod.yml` (usa imagens do GHCR).  
  Defina `IMAGE_TAG` (padrão `0.0.1`) e faça login no GHCR antes do `pull`.
...
3. Valide o acesso à tag que será usada:
   ```bash
   IMAGE_TAG=0.0.1
   docker pull ghcr.io/85dyones/motogestor-v2-gateway:${IMAGE_TAG}
   docker pull ghcr.io/85dyones/motogestor-v2-users:${IMAGE_TAG}
   docker pull ghcr.io/85dyones/motogestor-v2-management:${IMAGE_TAG}
   docker pull ghcr.io/85dyones/motogestor-v2-financial:${IMAGE_TAG}
   docker pull ghcr.io/85dyones/motogestor-v2-teamcrm:${IMAGE_TAG}
   docker pull ghcr.io/85dyones/motogestor-v2-ai:${IMAGE_TAG}
   ```
...
2. Suba apenas com imagens publicadas:
   ```bash
   IMAGE_TAG=0.0.1 docker compose -f docker-compose.prod.yml --env-file .env pull
   IMAGE_TAG=0.0.1 docker compose -f docker-compose.prod.yml --env-file .env up -d
   ```
3. Aplique migrations (se não usar o script `deploy_staging.sh`):
   ```bash
   IMAGE_TAG=0.0.1 docker compose -f docker-compose.prod.yml --env-file .env run --rm users-service alembic upgrade head
   IMAGE_TAG=0.0.1 docker compose -f docker-compose.prod.yml --env-file .env run --rm management-service alembic upgrade head
   IMAGE_TAG=0.0.1 docker compose -f docker-compose.prod.yml --env-file .env run --rm financial-service alembic upgrade head
   IMAGE_TAG=0.0.1 docker compose -f docker-compose.prod.yml --env-file .env run --rm teamcrm-service alembic upgrade head
   ```
4. Verifique health:
   ```bash
