...
- Disponibilizar um script único de deploy de staging que puxa as imagens `0.0.1`, sobe o `docker-compose.prod.yml` em modo staging e aplica `alembic upgrade head` antes de considerar o deploy ok.
...
- Ele carrega variáveis de `.env`, faz `docker compose --env-file .env -f docker-compose.prod.yml pull` (imagens tag `0.0.1`) e sobe `postgres`/`redis` primeiro.
