"""Reusable tenant guard middleware/decorator for Flask views."""
...
from .tenant import TENANT_ID_CLAIM
...
def _set_pg_tenant_scope(tenant_id: int) -> None:
    """Propagate tenant to Postgres RLS (ignored on SQLite)."""
...
def tenant_guard(path_key: str = "tenant_id", body_keys: Iterable[str] = ("tenant_id",)):
    """
    Decorator that enforces tenant consistency between JWT and request payload/path.

    - Requires JWT to be present.
    - Requires tenant_id in path or body.
    - Rejects if tenant_id doesn't match the token claim.
    - Injects ``g.current_tenant_id`` when successful and sets Postgres RLS GUC.
    """
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            claims = get_jwt() or {}
            token_tenant = claims.get(TENANT_ID_CLAIM)
            if token_tenant is None:
                raise ForbiddenError("Token não contém tenant_id.")

            request_tenant = _extract_request_tenant(path_key, body_keys)
            if request_tenant is None:
                raise ValidationError("tenant_id é obrigatório no path ou corpo.")

            if int(token_tenant) != int(request_tenant):
                return jsonify({"error": "tenant_id não corresponde ao token"}), 403
...
def inject_current_tenant_from_token(optional: bool = True) -> None:
    """Utility for ``before_request`` to set tenant context even when optional."""
